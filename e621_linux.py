...
def open_dir( o=False ):
    ...
    if o:
        if messagebox.askyesno( 'Open Folder?','Would you like to open the folder now?' ):
            os.system('xdg-open "%s"' % SAVE_FOLDER )
    else:
        os.system('xdg-open "%s"' % SAVE_FOLDER )
...
page_btn.place(relx=.435, rely=.305 )
...
cancel.place(relx=.125, rely=.3 )
cancel.configure( state="disabled" )

open_f = Button(window, text="Open Folder", command=open_dir )
open_f.place(relx=.215, rely=.3 )

image_e = Label( window, image = "" )
image_e.place(relx=0.525, rely=.049 )
...
prog_txt.place(relx=.01, rely=.39 )
file_size = Label( window, text=" ", anchor="e" )
file_size.place(relx=.39, rely=.39 )
prog_bar = Progressbar( window, length=395, style='green.Horizontal.TProgressbar')
prog_bar.place(relx=.01, rely=.46 )

prog2_bar = Progressbar( window, length=395, style='green.Horizontal.TProgressbar')
